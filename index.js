...
async function generate() {
    ...
    if (this.id === "generateCustomButton") {
        ...
        // if the theme is found within the database, retrieve comments
        if (allComments !== null) {
    ...
    // if a reroll is selected, a different group of comments under the same theme is retrieved
        ...
        if (allComments !== null) {
            if (index < allComments.comments.length) {
                comments = [];
                comments[0] = allComments.comments[index];
                comments[1] = allComments.comments[index + 1];
                comments[2] = allComments.comments[index + 2];
                comments[3] = allComments.comments[index + 3];
                await pause(1000);
            }
        }
    }

    // runs for random generation but checks for custom generation if comments are not returned
    if (comments !== null) {
    ...
    // if a custom theme does not exist OR a reroll is selected, call gemini to generate and store in database
        ...
        await storeCommentsInDatabase(comments);
        allComments = await getCommentsFromDatabase(theme);
...
// simple function to pause code to simulate loading
function pause(ms) {
...
function activateButton(button) {
    ...
    document.getElementById("generateCustomButton").disabled = true;
...
function deactivateButton(button) {
    ...
    document.getElementById("generateCustomButton").disabled = false;
