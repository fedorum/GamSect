...
async function generate() {
    ...
    // if a custom theme is inputted, check if it exists in the database first
    if (type == "custom") {
        comments = await getCommentsFromDatabase(theme, "custom");
        if (comments != null) {
    ...
    // if random generation is selected, retrieve a random theme and its related comments from the database
    if (type == "random") {
        comments = await getCommentsFromDatabase("random", "random");
        console.log("Theme and comments retrieved from database!");
    }

    // if the custom theme does not exist OR the generation is a reroll, call gemini to generate and store in database after
    if (comments == null) {
...
async function getCommentsFromDatabase(theme, type) {
    try {
        var action;
        switch (type) {
            case 'custom':
